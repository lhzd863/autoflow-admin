<template>
  <div>
    ...
    <div class="app-container">
      ...
      >
        <el-table-column label="StreamSys" min-width="150px">
          <template slot-scope="{row}">
            <span>{{ row.streamsys }}</span>
          </template>
        </el-table-column>
        <el-table-column label="StreamJob" min-width="150px">
          <template slot-scope="{row}">
            <span>{{ row.streamjob }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Description" min-width="50px">
          <template slot-scope="{row}">
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="margin-left:50px;">
          <el-form-item v-if="dialogStatus==='create'?true:false" label="StreamSys" prop="streamsys">
            <el-input v-model="temp.streamsys" />
          </el-form-item>
          <el-form-item v-if="dialogStatus==='create'?true:false" label="StreamJob" prop="streamjob">
            <el-input v-model="temp.streamjob" />
          </el-form-item>
          ...
          <el-form-item v-if="dialogStatus==='create'?false:true" label="StreamSys" prop="streamsys">
            <span> {{ temp.streamsys }} </span>
          </el-form-item>
          <el-form-item v-if="dialogStatus==='create'?false:true" label="StreamJob" prop="streamjob">
            <span> {{ temp.streamjob }} </span>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        streamsys: [{ required: true, message: 'stream sys is required', trigger: 'change' }],
        streamjob: [{ required: true, message: 'stream job is required', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    getList() {
      ...
      const url = global_system_data_url + '/flow/job/stream/ls?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const url = global_system_data_url + '/flow/job/stream/add?accesstoken=' + accesstoken
          fetch(url, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json;charset=UTF-8'
            ...
            body: JSON.stringify({
              ...
              sys: this.temp.sys,
              job: this.temp.job,
              streamsys: this.temp.streamsys,
              streamjob: this.temp.streamjob,
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const url = global_system_data_url + '/flow/job/stream/update?accesstoken=' + accesstoken
          fetch(url, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json;charset=UTF-8'
    ...
    deleteData(row, index) {
      ...
      const url = global_system_data_url + '/flow/job/stream/rm?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
