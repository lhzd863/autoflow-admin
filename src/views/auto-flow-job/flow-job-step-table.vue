<template>
  <div>
    ...
    <div class="app-container">
      ...
      >
        <el-table-column label="Sys" prop="job" sortable="custom" align="center" width="80" :class-name="getSortClass('+job')" :sort-orders="['ascending', 'descending']">
        ...
        <el-table-column label="Step" prop="step" sortable="custom" align="center" width="80" :class-name="getSortClass('+step')" :sort-orders="['ascending', 'descending']">
          <template slot-scope="{row}">
            <span>{{ row.step }}</span>
          </template>
        </el-table-column>
        <el-table-column label="Type" min-width="50px">
          <template slot-scope="{row}">
            <span>{{ row.type }}</span>
          </template>
        </el-table-column>
        <el-table-column label="Cmd" width="300px" align="center">
          <template slot-scope="{row}">
            <span>{{ row.cmd }}</span>
          </template>
        </el-table-column>
        <el-table-column label="Parameter" width="100px" align="center">
          <template slot-scope="{row}">
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="margin-left:50px;">
          ...
          <el-form-item v-if="dialogStatus==='create'?true:false" label="Step" prop="step">
            <el-select v-model="temp.step" class="filter-item" placeholder="Please select">
              <el-option v-for="item in stepOptions" :key="item.key" :label="item.label" :value="item.key" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item v-if="dialogStatus==='create'?false:true" label="Step" prop="step">
            <span> {{ temp.step }} </span>
          </el-form-item>
          ...
          <el-form-item label="Cmd" prop="cmd">
            <el-input v-model="temp.cmd" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="Please input" />
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      stepOptions: [{ label: 'Step-1', key: '1' }, { label: 'Step-2', key: '2' }, { label: 'Step-3', key: '3' }, { label: 'Step-4', key: '4' }, { label: 'Step-5', key: '5' }],
      typeOptions: [{ label: 'sh', key: 'sh' }, { label: 'bash', key: 'bash' }, { label: 'other', key: '' }],
      temp: {
        flowid: '',
        sys: undefined,
        job: '',
  ...
  methods: {
    getList() {
      ...
      const url = global_system_data_url + '/flow/job/cmd/ls?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
    ...
    sortByJob(order) {
      if (order === null) {
        this.tableQuery.sort = this.tableQuery.sort === '+job' ? '-job' : '+job'
      } else if (order === 'ascending') {
        this.tableQuery.sort = '+job'
      } else {
        this.tableQuery.sort = '-job'
      }
      if (this.tableQuery.sort === '+job') {
    ...
    sortByStep(order) {
      if (order === null) {
        this.tableQuery.sort = this.tableQuery.sort === '+step' ? '-step' : '+step'
      } else if (order === 'ascending') {
        this.tableQuery.sort = '+step'
      } else {
        this.tableQuery.sort = '-step'
      }
      if (this.tableQuery.sort === '+step') {
        this.tableQuery.pageData = this.tableQuery.pageData.sort((a, b) => (a.length === b.length) ? b.step.localeCompare(a.step) : (b.step - a.step))
      } else {
        this.tableQuery.pageData = this.tableQuery.pageData.sort((a, b) => (a.length === b.length) ? a.step.localeCompare(b.step) : (a.step - b.step))
      }
    },
    resetTemp() {
      this.temp = {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const url = global_system_data_url + '/flow/job/cmd/add?accesstoken=' + accesstoken
          fetch(url, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json;charset=UTF-8'
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const url = global_system_data_url + '/flow/job/cmd/update?accesstoken=' + accesstoken
          fetch(url, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json;charset=UTF-8'
    ...
    deleteData(row, index) {
      ...
      const url = global_system_data_url + '/flow/job/cmd/rm?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
