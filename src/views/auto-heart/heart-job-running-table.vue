    ...
    <el-row :gutter="8">
      <el-col :xs="{span: 24}" :sm="{span: 24}" :md="{span: 24}" :lg="{span: 24}" :xl="{span: 24}" style="padding-right:8px;margin-bottom:30px;">
        ...
        >
          ...
          <el-table-column label="MstId" prop="mstid" sortable="custom" align="center" width="150" :class-name="getSortClass('+mstid')" :sort-orders="['ascending', 'descending']">
            <template slot-scope="{row}">
              <span>{{ row.mstid }}</span>
            </template>
          </el-table-column>
          <el-table-column label="MstIp" min-width="120px">
            <template slot-scope="{row}">
              <span>{{ row.mip }}</span>
            </template>
          </el-table-column>
          <el-table-column label="MstPort" min-width="80px">
            <template slot-scope="{row}">
              <span>{{ row.mport }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="Sip" min-width="120px">
            <template slot-scope="{row}">
              <span>{{ row.sip }}</span>
            </template>
          </el-table-column>
          <el-table-column label="Sport" max-width="80px">
            <template slot-scope="{row}">
              <span>{{ row.sport }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      const url = global_system_data_url + '/mst/flow/routine/job/running/heart/ls?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
    ...
    deleteData(row, index) {
      ...
      this.listLoading = true
      const url = global_system_data_url + '/mst/flow/routine/job/running/heart/rm?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
