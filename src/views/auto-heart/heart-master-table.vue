    ...
    <el-row :gutter="8">
      <el-col :xs="{span: 24}" :sm="{span: 24}" :md="{span: 24}" :lg="{span: 24}" :xl="{span: 24}" style="padding-right:8px;margin-bottom:30px;">
        ...
        >
          ...
          <el-table-column label="FlowNum" min-width="80px">
            <template slot-scope="{row}">
              <span>{{ row.flownum }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      const url = global_system_data_url + '/mst/heart/ls?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
    ...
    deleteData(row, index) {
      ...
      this.listLoading = true
      const url = global_system_data_url + '/mst/heart/rm?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
