<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleStartFlow">
        Start Flow
      </el-button>
      <el-button class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-edit" @click="handleStopFlow">
        Stop Flow
      </el-button>
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleStartRoutine">
        Start Routine
      </el-button>
      <el-button class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-edit" @click="handleStopRoutine">
        Stop Routine
      </el-button>
    </div>
    <el-row :gutter="8">
      <el-col :xs="{span: 24}" :sm="{span: 24}" :md="{span: 24}" :lg="{span: 24}" :xl="{span: 24}" style="padding-right:8px;margin-bottom:30px;">
        ...
        >
          ...
          <el-table-column label="MstId" prop="mstid" sortable="custom" align="center" width="200" :class-name="getSortClass('+mstid')" :sort-orders="['ascending', 'descending']">
            <template slot-scope="{row}">
              <span>{{ row.mstid }}</span>
            </template>
          </el-table-column>
          <el-table-column label="FlowId" min-width="180px">
            <template slot-scope="{row}">
              <span>{{ row.flowid }}</span>
            </template>
          </el-table-column>
          <el-table-column label="RoutineId" min-width="100px">
            <template slot-scope="{row}">
              <span>{{ row.routineid }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="Port" max-width="80px">
            <template slot-scope="{row}">
              <span>{{ row.port }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="ProcessNum" min-width="80px">
            <template slot-scope="{row}">
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
          <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
            <el-form-item label="MstId" prop="mstid">
              <el-input v-model="temp.mstid" />
            </el-form-item>
            <el-form-item label="FlowId" prop="flowid">
              <el-input v-model="temp.flowid" />
            </el-form-item>
            <el-form-item v-if="(dialogStatus==='start-routine'||dialogStatus==='stop-routine')?true:false" label="RoutineId" prop="routineid">
              <el-input v-model="temp.routineid" />
            </el-form-item>
            <el-form-item v-if="(dialogStatus==='start-routine'||dialogStatus==='stop-routine')?false:true" label="ProcessNum" prop="processnum">
          ...
          <div slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="dialogStatus==='start-flow'?startFlowData():(dialogStatus==='stop-flow'?stopFlowData():(dialogStatus==='start-routine'?startRoutineData():(dialogStatus==='stop-routine'?startRoutineData():getList())))">
              Confirm
            </el-button>
          </div>
        </el-dialog>
...
export default {
  name: 'HeartMasterFlowRuntineTable',
  ...
  data() {
    return {
      ...
      processnumOptions: [{ label: '1', key: '1' }, { label: '2', key: '2' }, { label: '3', key: '3' }, { label: '4', key: '4' }, { label: '5', key: '5' }, { label: '6', key: '6' }, { label: '7', key: '7' }, { label: '8', key: '8' }, { label: '9', key: '9' }],
      temp: {
        mstid: '',
        flowid: '',
        routineid: '',
  ...
  methods: {
    getList() {
      ...
      const url = global_system_data_url + '/mst/flow/routine/heart/ls?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
    ...
    handleStartRoutine() {
      this.resetTemp()
      this.dialogStatus = 'start-routine'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
      })
    ...
    handleStopRoutine() {
      this.resetTemp()
      this.dialogStatus = 'stop-routine'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
      })
    ...
    deleteData(row, index) {
      ...
      this.listLoading = true
      const url = global_system_data_url + '/mst/flow/routine/heart/rm?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
    ...
    startFlowData() {
      ...
      this.listLoading = true
      const url = global_system_data_url + '/mst/instance/start?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
    ...
    stopFlowData() {
      ...
      this.listLoading = true
      const url = global_system_data_url + '/mst/instance/stop?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
    ...
    startRoutineData() {
      ...
      this.listLoading = true
      const url = global_system_data_url + '/mst/flow/routine/start?accesstoken=' + accesstoken
      fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
